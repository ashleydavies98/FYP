...
def app():
    ...
    st.write("PSO originates from Population-Based optimisation algorithms. These are algorithms that use a set of "
             "currently bad solutions to generate far better solutions. 'Population' refers to the current solution "
             "set, which better solutions are generated from.")

    st.subheader("How does PSO work?")

    st.write("Candidate solutions for a problem represented by Particles, are placed "
             ...
             "When using PSO, potential solutions are pit against a fitness value/function "
             "which determines a potential solution's quality for solving the given problem."
             "The main variant of PSO covered is Global Best PSO, which means that all Particles are "
             "one large neighbourhood, where Velocity is influenced by the Particle with the best known "
             "position found at any given time. However, there are multiple types that are discussed throughout.  \n")

    # create an expanding widget with relevant label
    expander = st.beta_expander(label='PSO and flocking behaviour')
    ...
    expander.write("Particles on the right are grouped and oscillate around in a similar fashion to the bird "
                   ...
                   "the best position found so far by the entire swarm for judging movement. This means the "
                   "Particles follow the strongest Particle, which is similar to how birds in a flock commonly follow "
                   "the strongest bird of the flock.")
    ...
    st.info(
            ...
            "- Types of PSO and how they effect results.  \n"
